...
make_kde_grid <- function(x, bw_adjust = 0.80, grid_n = 512L) {
  ...
  support <- sqrt(5) * h
  # A Cauchy draw can make an extrema-based grid arbitrarily wide and leave
  # too few quadrature points over the mixture body.  Use the robust window
  # from the original Scenario 2 code while retaining the Epanechnikov KDE.
  center <- stats::median(x)
  scale <- suppressWarnings(stats::IQR(x) / 1.349)
  if (!is.finite(scale) || scale <= 1e-6)
    scale <- suppressWarnings(stats::mad(x, constant = 1.4826))
  if (!is.finite(scale) || scale <= 1e-6) scale <- safe_scale(x)
  half_width <- max(4 * scale, 3 * h, 6)
  grid <- seq(
