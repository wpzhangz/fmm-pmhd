...
# Reproducible Scenario 1 simulation for Table 1.
#
# Full run (Linux/Windows):
#   Rscript scenario1.R --B=100 --n=100,200,500 --cores=8
#
# Smoke test:
#   Rscript scenario1.R --quick --force
#
# Every replication performs the complete analysis in one pass:
#   data generation -> PMHD-MCP path -> penalized support-gap completion ->
#   trimmed parsimonious elbow -> metrics, together with GM-MLE and
#   AEPD-MLE-HQ on the same data.
#
...
.scenario1_script_file <- local({
  ...
  if (is.null(candidate) || !nzchar(candidate))
    candidate <- file.path(getwd(), "scenario1.R")
  normalizePath(candidate, winslash = "/", mustWork = FALSE)
})
.scenario1_script_dir <- dirname(.scenario1_script_file)
.scenario1_base_file <- file.path(.scenario1_script_dir, "scenario1_base.R")
if (!file.exists(.scenario1_base_file))
  stop("scenario1_base.R must be beside scenario1.R.")
source(.scenario1_base_file, local = FALSE, chdir = TRUE)
...
scenario1_settings <- list(
  s1a = list(
    label = "Scenario 1(a)", seed = 10101L,
    pi = c(0.3, 0.4, 0.3), mu = c(-4, 0, 4),
    sigma = c(0.7, 0.9, 0.7),
    alpha = c(2.0, 2.0, 2.0), tau = c(0.50, 0.50, 0.50)
  ),
  s1b = list(
    label = "Scenario 1(b)", seed = 20201L,
    pi = c(0.3, 0.4, 0.3), mu = c(-4, 0, 4),
    sigma = c(0.7, 0.9, 0.7),
    alpha = c(1.8, 1.7, 1.8), tau = c(0.45, 0.50, 0.55)
  ),
  s1c = list(
    label = "Scenario 1(c)", seed = 30301L,
    pi = c(0.3, 0.4, 0.3), mu = c(-4, 0, 4),
    sigma = c(0.7, 0.9, 0.7),
    alpha = c(1.25, 1.15, 1.25), tau = c(0.32, 0.38, 0.44)
  )
)

table1_methods <- c("PMHD-MCP", "GM-MLE", "AEPD-MLE-HQ")
...
# -------------------------------------------------------------------------
# Implemented PMHD-MCP selector used in Table 1
# -------------------------------------------------------------------------
...
one_replication <- function(b, truth, evaluation, config, seed_base) {
  data_seed <- seed_base + as.integer(b)
  set.seed(data_seed)
  x <- rmix_aepd(
  ...
  list(
    "PMHD-MCP" = fit_pmhd_mcp(
      x, truth, evaluation, config, seed = data_seed
    ),
...
# -------------------------------------------------------------------------
# Summaries, Table 1, checkpointing, and CLI
# -------------------------------------------------------------------------
...
summarize_method <- function(replicates, method, setting, label, n) {
  ...
  data.frame(
    setting = setting, scenario = label, n = n, method = method,
...
write_table1 <- function(summary, path, B) {
  fmt <- function(x, digits) {
    if (!is.finite(x)) "--" else formatC(x, format = "f", digits = digits)
  }
  lines <- c(
    sprintf("%% Scenario 1 Table 1; B=%d; generated by scenario1.R", B),
    "\\begin{tabular}{llcccccccc}",
    "\\toprule",
    paste(
      "$n$ & Method & $P(\\hat K=K_0)$ & $\\bar K$ & $H$ & ISE",
      "& $\\mathrm{MSE}_\\mu$ & $\\mathrm{MSE}_\\sigma$",
      "& $\\mathrm{MSE}_\\alpha$ & $\\mathrm{MSE}_\\tau$ \\\\"
    ),
    "\\midrule"
  )
  settings <- unique(summary$setting)
  for (s in seq_along(settings)) {
    block <- summary[summary$setting == settings[s], , drop = FALSE]
    lines <- c(
      lines,
      sprintf(
        "\\multicolumn{10}{l}{\\textit{%s}} \\\\",
        block$scenario[1L]
      ),
      "\\midrule"
    )
...
run_scenario1 <- function(
    ...
    settings = names(scenario1_settings),
    out_dir = file.path(.scenario1_script_dir, "scenario1_results"),
    force = FALSE, checkpoint_every = NULL,
    pmhd_grid_n = 512L, evaluation_grid_n = 4096L,
    pmhd_nstart = 20L, gap_nstart = 6L, aepd_nstart = 20L,
    quick = FALSE) {
  settings <- intersect(names(scenario1_settings), settings)
  if (!length(settings)) stop("No recognized setting was requested.")
  ...
  version <- "scenario1_table1_direct_v1"
  rows <- list()

  for (setting_name in settings) {
    truth <- scenario1_settings[[setting_name]]
    evaluation <- truth_evaluation_grid(
      truth, grid_n = evaluation_grid_n
    )
    for (n in as.integer(n_values)) {
      n_slot <- match(n, c(100L, 200L, 500L))
      if (is.na(n_slot))
        stop("Table 1 supports n=100,200,500 only.")
      seed_base <- truth$seed + 100000L * n_slot
      config <- list(
        n = n, K_max = 5L,
      ...
      metadata <- list(
        version = version, setting = setting_name, n = n, B = B,
        seed_base = seed_base, methods = table1_methods,
        truth = truth, config = config,
        evaluation_grid_n = evaluation_grid_n
      )
      cache_file <- file.path(
        cache_dir,
        sprintf("%s_%s_n%d_B%d.rds", version, setting_name, n, B)
      )
      ...
      message(sprintf(
        "%s, n=%d: resumed %d/%d",
        truth$label, n, B - length(missing), B
      ))
      if (length(missing)) {
        batches <- split(
          missing,
        ...
        if (cores > 1L && length(missing) > 1L) {
          ...
          scenario_file <- .scenario1_script_file
      ...
      for (method in table1_methods) {
        row <- summarize_method(
          replicates, method, setting_name, truth$label, n
        )
      ...
      utils::write.csv(
        partial, file.path(out_dir, "scenario1_summary_partial.csv"),
        row.names = FALSE, na = ""
      )
    }
  }
  ...
  summary_file <- file.path(out_dir, "scenario1_summary.csv")
  table_file <- file.path(out_dir, "scenario1_table1.tex")
  utils::write.csv(summary, summary_file, row.names = FALSE, na = "")
  write_table1(summary, table_file, B)
  saveRDS(
    list(
      version = version, settings = scenario1_settings[settings],
      summary = summary
    ),
    file.path(out_dir, "scenario1_run_manifest.rds")
  )
  message("Written:")
