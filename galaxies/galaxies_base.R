...
make_kde_grid <- function(
    x, grid_n = 512L,
    bandwidth_method = getOption("pmhd.bandwidth_method", "sj-dpi")) {
  x <- as.numeric(x)
  ...
  h <- switch(
    ...
    "nrd0-0.8" = 0.8 * stats::bw.nrd0(x),
    "gaussian-nrd0-0.8" = 0.8 * stats::bw.nrd0(x)
  )
  if (!is.finite(h) || h <= 0)
    stop("The selected KDE bandwidth is invalid.")
  gaussian <- bandwidth_method != "nrd0-0.8"
  support <- if (gaussian) 6 * h else sqrt(5) * h
  grid <- seq(min(x) - support, max(x) + support, length.out = grid_n)
  u <- outer(grid, x, "-") / h
  kernel_values <- if (gaussian)
    stats::dnorm(u) else epanechnikov_kernel(u)
  phat <- rowMeans(matrix(kernel_values, nrow = length(grid))) / h
  w <- trapz_weights(grid)
  phat <- normalize_grid_density(phat, w)
  list(
    grid = grid, w = w, phat = phat, sqrt_phat = sqrt(phat),
    bandwidth = h,
    bandwidth_method = switch(
      bandwidth_method,
      "sj-dpi" = "Sheather-Jones direct plug-in",
      "nrd0-0.8" = "0.8 * bw.nrd0",
      "gaussian-nrd0-0.8" = "0.8 * bw.nrd0"
    ),
    kernel = if (gaussian) "gaussian" else "epanechnikov",
...
fit_pmhd_path <- function(x, K, lambda_grid = default_lambda_grid(),
                          ...
                          lambda_upper_cap = max(lambda_grid),
  ...
  if (!is.finite(lambda_upper_cap) ||
      lambda_upper_cap < max(lambda_grid))
    stop("lambda_upper_cap must be at least max(lambda_grid).")
  nstart <- as.integer(nstart)
  max_grid_expansions <- as.integer(max_grid_expansions)
  checkpoint <- NULL
  if (!is.null(checkpoint_file) && resume_checkpoint &&
    ...
    if (!isTRUE(all.equal(
      ...
      stop(
        "Path checkpoint settings differ: ", checkpoint_file,
        ". Use --force or remove that checkpoint."
      )
  }
  if (!is.null(checkpoint)) {
    ...
    progress_message(
      "Resuming path checkpoint with ", length(records),
      " completed lambda value(s): ", checkpoint_file
    )
  } else {
    progress_message(
      "Constructing KDE: n=", length(x), ", grid=", grid_n,
      ", method=",
      getOption("pmhd.bandwidth_method", "sj-dpi"), "."
    )
    kde <- make_kde_grid(x, grid_n)
    records <- list()
  }
  fit_control$support_tol <- fit_control$support_tol %||% support_tol

  key_of <- function(lambda) sprintf("%.17g", lambda)
  expansion <- as.integer(checkpoint$grid_expansions %||% 0L)
  save_path_checkpoint <- function(stage) {
    ...
    saveRDS(
      list(
        checkpoint_id = checkpoint_id, kde = kde,
        records = records, grid_expansions = expansion,
        stage = stage, saved_at = Sys.time()
      ),
      temporary, compress = "gzip"
    )
    if (!file.copy(temporary, checkpoint_file, overwrite = TRUE))
      stop("Could not update path checkpoint: ", checkpoint_file)
    unlink(temporary)
    invisible(NULL)
  }
  adaptive_nstart <- max(
  ...
  fit_missing_lambda <- function(lambda, starts_at_lambda = nstart) {
    ...
    save_path_checkpoint(
      paste0("lambda=", format(lambda, digits = 12))
    )
    invisible(NULL)
  }
  admissible_fits <- function(record) {
    if (is.null(record$fits)) return(list())
    Filter(
      function(fit)
        is.list(fit) && !inherits(fit, "pmhd_error") &&
        isTRUE(fit$admissible) && is.finite(fit$Q %||% NA_real_),
      record$fits
    )
  }
  select_monotone_branch <- function() {
    lambdas <- sort(vapply(records, function(z) z$lambda, numeric(1)))
    candidate_sets <- lapply(lambdas, function(lambda)
      admissible_fits(records[[key_of(lambda)]]))
    if (any(!lengths(candidate_sets)))
      stop("At least one lambda has no admissible candidate fit.")

    # Only objective differences within a lambda matter. Subtracting the
    # pointwise minimum removes the lambda-dependent additive MCP constant.
    local_cost <- lapply(candidate_sets, function(candidates) {
      objective <- vapply(candidates, function(fit) fit$Q, numeric(1))
      scale <- max(stats::mad(objective), 1e-8)
      (objective - min(objective)) / scale
    })
    dynamic_cost <- vector("list", length(lambdas))
    predecessor <- vector("list", length(lambdas))
    dynamic_cost[[1L]] <- local_cost[[1L]]
    predecessor[[1L]] <- rep(NA_integer_, length(candidate_sets[[1L]]))

    if (length(lambdas) > 1L) {
      for (j in 2:length(lambdas)) {
        previous_order <- vapply(
          candidate_sets[[j - 1L]], function(fit) fit$Khat, integer(1)
        )
        current_order <- vapply(
          candidate_sets[[j]], function(fit) fit$Khat, integer(1)
        )
        dynamic_cost[[j]] <- rep(Inf, length(current_order))
        predecessor[[j]] <- rep(NA_integer_, length(current_order))
        for (k in seq_along(current_order)) {
          feasible <- which(
            previous_order >= current_order[k] &
              is.finite(dynamic_cost[[j - 1L]])
          )
          if (length(feasible)) {
            best <- feasible[which.min(dynamic_cost[[j - 1L]][feasible])]
            dynamic_cost[[j]][k] <-
              dynamic_cost[[j - 1L]][best] + local_cost[[j]][k]
            predecessor[[j]][k] <- best
          }
        }
        if (!any(is.finite(dynamic_cost[[j]])))
          stop(
            "No admissible global nonincreasing support branch reaches ",
            "lambda=", format(lambdas[j], digits = 12),
            ". Increase nstart and resume from the path checkpoint."
          )
      }
    }

    chosen_index <- integer(length(lambdas))
    chosen_index[length(lambdas)] <- which.min(
      dynamic_cost[[length(lambdas)]]
    )
    if (length(lambdas) > 1L) {
      for (j in seq(length(lambdas), 2L))
        chosen_index[j - 1L] <- predecessor[[j]][chosen_index[j]]
    }
    ...
    for (j in seq_along(lambdas)) {
      ...
      key <- key_of(lambda)
      record <- records[[key]]
      chosen <- candidate_sets[[j]][[chosen_index[j]]]
      if (is.null(record$fit_unconstrained))
        record$fit_unconstrained <- record$fit
      repaired <- !isTRUE(all.equal(
        chosen$pi, record$fit_unconstrained$pi,
        tolerance = 0, check.attributes = FALSE
      ))
      record$fit <- chosen
      record$success <- TRUE
      record$branch_repaired <- repaired
      record$reason <- if (repaired)
        "selected on the nonincreasing support branch" else "success"
      records[[key]] <<- record
      repairs <- repairs + as.integer(repaired)
    }
    progress_message(
      "Global nonincreasing support branch selected; ",
      repairs, " lambda fit(s) changed from their unconstrained best."
    )
    save_path_checkpoint("nonincreasing branch selected")
    invisible(repairs)
  }
  build_path <- function() {
    lambdas <- sort(vapply(records, function(z) z$lambda, numeric(1)))
    rows <- lapply(lambdas, function(lambda) {
      ...
      data.frame(
        ...
        branch_repaired = isTRUE(z$branch_repaired),
        stringsAsFactors = FALSE
      )
    })
  ...
  progress_message(
    "Starting PMHD-MCP path: ", length(lambda_grid),
    " lambda values, ", nstart, " starts per value."
  )
  positive <- lambda_grid[lambda_grid > 0]
  initial_sequence <- if (length(positive) && any(lambda_grid == 0))
    c(min(positive), 0, setdiff(positive, min(positive))) else lambda_grid
  for (j in seq_along(initial_sequence)) {
    lambda <- initial_sequence[j]
    progress_message(
      "Path ", j, "/", length(initial_sequence),
      ": lambda=", format(lambda, digits = 8)
    )
    fit_missing_lambda(lambda)
    record <- records[[key_of(lambda)]]
    if (isTRUE(record$success)) {
      progress_message(
        "Path ", j, "/", length(initial_sequence),
        " completed: K=", record$fit$Khat,
        ", H2=", format(record$fit$H2, digits = 7)
      )
    } else {
      progress_message(
        "Path ", j, "/", length(initial_sequence), " failed."
      )
    }
  }
  if (length(positive) && any(lambda_grid == 0)) {
    zero_key <- key_of(0)
    near_key <- key_of(min(positive))
    zero_record <- records[[zero_key]]
    if (!isTRUE(zero_record$reverse_warm_done)) {
      progress_message(
        "Reverse warm-start refinement for lambda=0 from lambda=",
        format(min(positive), digits = 8), "."
      )
      reverse <- fit_pmhd_at_lambda(
        x = x, K = K, lambda = 0, kde = kde,
        nstart = max(adaptive_nstart, 2L), gamma = gamma,
        warm = records[[near_key]]$fits,
        seed = seed + 6000011L,
        fit_control = fit_control
      )
      if (isTRUE(reverse$success)) {
        zero_record$fits <- c(zero_record$fits, reverse$fits)
        good <- admissible_fits(zero_record)
        objective <- vapply(good, function(fit) fit$Q, numeric(1))
        zero_record$fit <- good[[which.min(objective)]]
        zero_record$success <- TRUE
      }
      zero_record$reverse_warm_done <- TRUE
      records[[zero_key]] <- zero_record
      save_path_checkpoint("lambda=0 reverse warm-start completed")
    }
  }
  select_monotone_branch()
  path <- build_path()
  ...
  while (expansion < max_grid_expansions) {
    ...
    if (nrow(compressed) < 3L ||
      ...
      new_lambda <- c(
        new_lambda, min(1.75 * upper, lambda_upper_cap)
      )
    }
    new_lambda <- unique(new_lambda[
      is.finite(new_lambda) & new_lambda <= lambda_upper_cap
    ])
    ...
    progress_message(
      "Adaptive lambda refinement ", expansion, ": ",
      length(new_lambda), " additional values."
    )
    for (j in seq_along(new_lambda)) {
      lambda <- new_lambda[j]
      progress_message(
        "Refinement ", expansion, ", ", j, "/", length(new_lambda),
        ": lambda=", format(lambda, digits = 8)
      )
      ...
      if (isTRUE(record$success))
        progress_message(
          "Refinement fit completed: K=", record$fit$Khat,
          ", H2=", format(record$fit$H2, digits = 7)
        )
      else
        progress_message("Refinement fit failed.")
    }
    select_monotone_branch()
    path <- build_path()
    compressed <- compress_h_path(path)
  ...
  progress_message(
    "PMHD-MCP path completed: ", nrow(path),
    " fitted lambda values and ", nrow(compressed), " attained orders."
  )
  list(
